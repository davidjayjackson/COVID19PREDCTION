...
```{r, echo=FALSE}
df1 <- dbGetQuery(db,"select * from JHDEATHS")
df1$Reported <- lubridate::ymd(df1$YMD)
...
##
(399163 - sum(df1death$DEATHS)) / 83  # Feb 1, 2021
sum(df1death$DEATHS) + (mean((df1death$DEATHS)) * 83) ## Feb.1 , 2021(389087)
##
## 
```
...
ggplot(df1death) +geom_line(aes(x=Reported,y=DEATHS),lwd=2,col="blue") + 
  ...
  labs(title="US Daily Total Deaths(2020-03-01 - 2020-10-31)",
       subtitle="Daily Avg (Red:934);Feb 1(Grenn: 1921/399163))"
       ,x="Date Reported",y="Daily Deaths") +
  geom_hline(yintercept=936,lwd=1.5,col="red") +
...
ggplot(df1death) +geom_smooth(aes(x=Reported,y=DEATHS,col="Loess"),span=0.1) +
  ...
  labs(title="US Daily Total Deaths(2020-03-01 - 2020-10-31)",
       subtitle="Daily Avg (Red:933);Feb 1(Green: 1853/399163))"
       ,x="Date Reported",y="Daily Deaths") +
  geom_hline(yintercept=1812,lwd=1.5,col="green") +
  geom_hline(yintercept=933,lwd=1.5,col="red") 
