...
##
(399163 - sum(df2death$DEATHS)) / 73  # Feb 1, 2021
sum(df2death$DEATHS) + (mean((df2death$DEATHS)) * 73) ## Feb.1 , 2021(389087)
##
## 
```
...
ggplot(df2death) +geom_line(aes(x=Reported,y=DEATHS),lwd=2,col="blue") + 
  ...
  labs(title="US Daily Total Deaths(2020-03-01 - 2020-10-31)",
       subtitle="Daily Avg (Red:956);Feb 1(Grenn: 1921/399163))"
       ,x="Date Reported",y="Daily Deaths") +
  geom_hline(yintercept=956,lwd=1.5,col="red") +
...
```{r}
##
(399163 - sum(df2death$DEATHS)) / 101  # Feb 1, 2021
sum(df2death$DEATHS) + (mean((df2death$DEATHS)) * 101) ## Feb.1 , 2021(389087)
##
## 
```
```{r}
...
ggplot(df2death) +geom_line(aes(x=Reported,y=DEATHS),lwd=2,col="blue") + 
  ...
  scale_y_continuous(labels = comma) +
   theme(axis.text.x=element_text(angle =- 45, vjust = 0.5)) +
  labs(title="US Daily Total Deaths(2020-03-01 - 2020-10-31)",
       subtitle="Daily Avg (Red:952);Feb 1(Grenn: 1433/399163))"
       ,x="Date Reported",y="Daily Deaths") +
  geom_hline(yintercept=956,lwd=1.5,col="red") +
    geom_hline(yintercept=1433,lwd=1.5,col="green")
